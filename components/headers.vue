    ...
    <div class="header min_wrapper_1200">
      <div class="headerCon wrapper_1200 acea-row row-between-wrapper">
        <a href="javascript:void(0);" rel="sidebar" @click="AddFavorite()"
          ><span class="iconfont icon-shoucang2"></span>收藏本站</a
        >
        <div class="user acea-row row-middle">
          <nuxt-link
            :to="{ path: '/user', query: { type: 0 } }"
          ...
          <nuxt-link
            :to="{ path: '/user/messageCenter?type=2' }"
            class="item service_color"
          ...
          <nuxt-link
            :to="{ path: '/user/orderList', query: { type: 1 } }"
            class="item"
            >我的订单</nuxt-link
          >
          <nuxt-link
            :to="{ path: '/user/balance', query: { type: 3 } }"
            class="item"
            >我的余额</nuxt-link
          >
          <nuxt-link
            to="/shoppingCart"
            class="cartNum"
            :class="$store.state.cartnumber > 0 ? 'on' : ''"
            ><span class="iconfont icon-dingbu-gouwuche"></span>购物车 ({{
          ...
          <div class="list">
            <nuxt-link
              :to="
                item.url === 1
                  ? $auth.loggedIn
                    ? { path: '/user/userCoupon', query: { type: 6 } }
                    : '/login'
                  : item.url
              "
              class="item"
