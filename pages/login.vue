<template>
    <div class="login">
      ...
      <div class="loginBg min_wrapper_1200">
        <div class="wrapper" v-show="current === 1">
          ...
          <div class="item verificat acea-row row-between-wrapper">
            ...
            <button class="code font-color" :disabled="disabled" :class="disabled === true ? 'on' : ''" @click="code">
              {{ text }}
            </button>
          </div>
        ...
        <div class="wrapper" v-show="current === 2">
          ...
          <div class="item phone acea-row row-middle">
            <div class="number">+86</div>
            <input type="text" placeholder="请输入手机号" maxlength="11" v-model="account">
          </div>
          <div class="item pwd">
      ...
      <div class="footer wrapper_1200">
        ...
        <div class="record">{{info.copyright}}<a href="http://www.miit.gov.cn/" target="_blank" class="num">{{info.record_No}}</a></div>
      </div>
    </div>
</template>
  ...
  import sendVerifyCode from "@/mixins/SendVerifyCode";
    export default {
      ...
      async asyncData({$axios,query}){
        ...
        return { keyCode:keyCode.key, info:companyInfo,appidNum:appidNum.appid,codes:query.code || '' }
      },
      fetch({ store}) {
